...
* получает из АТОЛ данные о статусе регистрации счета/возврата
  ...
  * вручную из админки кнопкой на странице Счета или Возврата
  * консольной командой `mygento:atol:update-one {$uuid}` или `mygento:atol:update-all {$storeId}`
...
* `mygento:atol:update-all` - Только Атол. Запрашивает данные о статусе всех отправок со статусом `wait` для указанного стора. Аргументы: StoreID
* `mygento:atol:update-one` - Только Атол. Запрашивает данные о статусе указанной отправки. Аргументы: UUID
