...
class Mygento_Kkm_Model_Vendor_Atol3 extends Mygento_Kkm_Model_Vendor_AtolAbstract implements Mygento_Kkm_Model_Vendor_VendorInterface
{
    ...
    {
        return $this->getUrl() . $this->getConfig('general/group_code') . '/' . $operation . '?tokenid=' . $this->getToken();
    }

    protected function getUpdateStatusUrl($uuid)
    {
        return  $this->getUrl() . $this->getConfig('general/group_code') . '/' . self::OPERATION_GET_REPORT . '/' . $uuid . '?tokenid=' . $this->getToken();
    }
